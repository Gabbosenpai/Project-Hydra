...
var robot_texts = {
	...
	"fire_hydrant": "Non dovrai aspettare mai più i pompieri! Le braccia del Fire Hydrant sono in grado di spruzzare getti d’acqua ad alta pressione per contrastare anche le fiamme più ostinate e, grazie alle sue molle, il fuoco ad alta quota non è più un problema! I pompieri sono scettici a causa dell’elevata probabilità che saltelli via a domare mozziconi di sigaretta roventi sui marciapiedi (acqua non inclusa)."
}
var clicked: bool
